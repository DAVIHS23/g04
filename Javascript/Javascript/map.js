...
const projection = d3.geoMercator().translate([width / 2, height / 1.5]);
const path = d3.geoPath().projection(projection);

let routeDelays = {};
let colorScale = d3.scaleLinear().domain([-1, 30, 476]).range(["yellow", "orange", "red"]);
    ...
    d3.csv("Data/Flights.csv").then(data => {
        ...
        data.forEach(d => {
            ...
            if ((selectedSeason === "All" || d.SEASON === selectedSeason) &&
                ...
                // Manually extract hours and minutes from the "HH:MM" format
                const flightTimeParts = d.FLIGHT_TIME.match(/(\d{2}):(\d{2})/);
                if (flightTimeParts) {
                    ...
                    const totalFlightTimeMinutes = (hours * 60) + minutes;
                    routes[routeKey].totalFlightTime += totalFlightTimeMinutes;
                }
            }
        });
        ...
        let routeData = Object.values(routes).map(route => {
            ...
            // Calculate average flight time in minutes and convert it to "HH:MM" format
            const averageFlightTimeMinutes = route.totalFlightTime / route.count;
            const averageHours = Math.floor(averageFlightTimeMinutes / 60);
            const averageMinutes = Math.floor(averageFlightTimeMinutes % 60);

            route.averageFlightTime = `${String(averageHours).padStart(2, '0')}:${String(averageMinutes).padStart(2, '0')}`;
...
function getColorScale(data) {
    ...
    const colorRange = ["#0041C2", "#00ff00", "#8B0000"];
...
function updateLegend(data) {
    ...
    const getColorScale = function () {
        ...
        // Define the range of colors you want to use
        const colorRange = ["yellow", "orange", "red"]; // Modify this array as needed
    ...
    const legendLabels = [`${minDelay.toFixed(2)} min delayed`, "30 min delayed", `${maxDelay.toFixed(2)} min delayed`];
    legend
